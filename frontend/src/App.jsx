...
export default function App({ config }) {
  ...
  return <div className="map-editor react-map-editor">
    <section className="card map-canvas-panel" aria-labelledby="map-heading"><h2 id="map-heading">澎南區地圖</h2><div id="map-capture-area" ref={captureRef} style={{ backgroundColor: state.mapBackgroundColor }}><MapCanvas geojson={geojson} layers={state.layers} colors={state.villageColors} backgroundColor={state.mapBackgroundColor} labelPositions={state.labelPositions} titleSettings={state.titleSettings} selected={state.selected} onSelect={(value) => dispatch({ type: "select", value })} onLabelMove={(id, position) => dispatch({ type: "label", id, position })} onTitleMove={(position) => dispatch({ type: "title", changes: { position } })} onMap={setMap} zoom={zoom} onZoom={setZoom}/><ChoroplethLegend layer={activeChoropleth}/></div><Legend layers={state.layers}/>

...
export default function App({ config }) {
  ...
  useEffect(() => { (async () => {
    ...
    dispatch({ type: "hydrate", payload: { year: availableYears.at(-1) || null, layers: mergeLayers(custom, stored.layers, [], genderColors), villageColors: colors, mapBackgroundColor: stored.mapBackgroundColor || DEFAULT_MAP_BACKGROUND_COLOR, labelPositions: stored.labelPositions, titleSettings: stored.titleSettings } }); hydrated.current = true;
  })(); }, []);
  ...
  useEffect(() => { if (hydrated.current) saveWorkspace(localStorage, state); }, [state.layers, state.villageColors, state.mapBackgroundColor, state.labelPositions, state.titleSettings]);
  const active = activeSelection(state), activeLayer = state.layers.find((layer) => layer.id === active?.layerId), activeValues = activeLayer?.values?.[active?.village];
  ...
  const changeLayerColor = async (layer, series, color) => {
    const previous = series.color;
    dispatch({ type: "seriesColor", layerId: layer.id, seriesId: series.id, color });
    try { await api.updateColors(layer.id, { [series.id]: color }); }
    catch (error) {
      dispatch({ type: "seriesColor", layerId: layer.id, seriesId: series.id, color: previous, ifColor: color });
      dispatch({ type: "status", message: `「${layer.name}」顏色儲存失敗：${error.message}` });
    }
  };
  const resetColors = () => {
  ...
  return <div className="map-editor react-map-editor">
    <section className="card map-canvas-panel" aria-labelledby="map-heading"><h2 id="map-heading">澎南區地圖</h2><div id="map-capture-area" ref={captureRef} style={{ backgroundColor: state.mapBackgroundColor }}><MapCanvas geojson={geojson} layers={state.layers} colors={state.villageColors} backgroundColor={state.mapBackgroundColor} labelPositions={state.labelPositions} titleSettings={state.titleSettings} active={active} onHover={(value) => dispatch({ type: "hover", value })} onToggle={(value) => dispatch({ type: "toggleSelect", value })} onClearSelection={() => dispatch({ type: "clearSelect" })} onLabelMove={(id, position) => dispatch({ type: "label", id, position })} onTitleMove={(position) => dispatch({ type: "title", changes: { position } })} onMap={setMap} zoom={zoom} onZoom={setZoom}/><ChoroplethLegend layer={activeChoropleth}/></div><Legend layers={state.layers}/>
      <div className="map-info"><strong>區域資訊</strong>{active && activeLayer ? <div><div><strong>{active.village}</strong> · {activeLayer.name}</div><dl>{activeLayer.series?.map((series) => <div key={series.id}><dt><i style={{ background: series.color }}/>{series.name}</dt><dd>{activeValues?.[series.id] ?? "—"}{activeLayer.source?.unit || ""}</dd></div>)}</dl><p className="hint">來源：{activeLayer.source?.type === "processed_data" ? `${activeLayer.source.year || state.year || "—"} 年／${activeLayer.source.data_type || "人口資料"}／${activeLayer.source.gender || "全部"}` : "CSV 上傳"}</p></div> : <p className="hint">將滑鼠移到圖表或區域上查看資料；觸控裝置可點按選取，再次點按或點擊空白處取消。</p>}</div>
    </section>
    <aside className="map-workspace" aria-label="地圖工作區">
      <details className="card map-workspace-section" open><summary>圖層工作區</summary><div className="map-workspace-section-body"><p className="hint">清單最上方為地圖最上層；可拖曳或使用上下按鈕排序。</p><label className="field">內建資料年份<select value={state.year || ""} onChange={(event) => dispatch({ type: "year", year: event.target.value })}>{years.map((year) => <option key={year}>{year}</option>)}</select></label><LayerPanel layers={state.layers} dispatch={dispatch} onDelete={deleteLayer} onColorChange={changeLayerColor} titleSettings={state.titleSettings}/><div className="map-layer-status" role="status">{state.status.join(" ")}</div></div></details>
